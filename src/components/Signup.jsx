...
// moving action function outside of the functional component, will make it more easy and when it will become more complex it will easy to use, outside of it

// function signupAction(formData) { it will access only state value errors only
function signupAction(prevFormState, formData) { // this will initialize state value to null first, as we have passed { error : null }
  ...
  if (errors.length > 0) {
    return {
      errors, enteredValues: {
        email, password, confirmPassword, firstName, lastName, role, acquisitionChannel, terms
      }
    }
  }

  return { errors: null } // we are not setting enteredValues over here because after form submisssion it will automatically set to {}
}
...
export default function Signup() {
  ...
  // formAction -> signUpAction, formState -> return values by signUpAction , pending -> form is being submitted or not (mostly async action)

  return (
    <form action={formAction}>
      {/* action have to be formAction which is representing signupAction */}
